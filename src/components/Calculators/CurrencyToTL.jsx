...
const CurrencyToTL = () => {
    ...
    const handleProductSelect = (e) => {
        ...
        if (prod) {
            setKur(prod.alis); // For Currency to TL, we usually use 'Alış' price (buying from customer)
        }
    };
    ...
    const hesapla = () => {
        ...
        const baseTl = m * k;
        const kdvAmount = baseTl * o;
        const total = baseTl + kdvAmount;

        setRawSonuc(total);
        setSonuc(total.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
    };
    ...
    return (
        <div className="calc-card" style={{ margin: '6px 16px', padding: '12px', background: '#fff' }}>
            <h3 style={{ fontSize: '12px', fontWeight: '600', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span style={{ background: 'var(--bg)', width: '20px', height: '20px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px' }}>5</span>
                Döviz → TL
            </h3>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                <div>
                    <label style={{ fontSize: '10px', color: 'var(--text-secondary)', display: 'block', marginBottom: '4px' }}>Döviz Miktarı</label>
                    <input className="input" type="text" value={miktar} onChange={(e) => setMiktar(e.target.value)} placeholder="0,00" style={{ height: '34px', fontSize: '12px' }} />
                </div>
                <div>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-end' }}>
                <div style={{ flex: 1 }}>
                    ...
                    <select className="input" value={selectedProduct} onChange={handleProductSelect} style={{ height: '34px', fontSize: '12px' }}>
                        ...
                        {currencies.map((c, i) => <option key={i} value={c.urun}>{c.urun} ({c.alis})</option>)}
                    </select>
                </div>
                <div style={{ flex: 1 }}>
